...
const IndependentAboutUs = () => {
    ...
    return (
        <IndependentDashboardLayout title="About Us"
        >
                ...
                <div className="flex flex-wrap w-full bg-[#D9D9D9] rounded-[80px] overflow-hidden mb-6 p-1 justify-center sm:justify-between gap-2 sm:gap-0 sm:h-[60px]">
                    {['About Us', 'Privacy Policy', 'Terms Of Service', 'Contact Us', 'Cookies', 'FAQs'].map((item, index) => (
                        <Link key={index} to={routeMapping[item]}>
                            <button
                                className={`py-1 px-5 sm:px-7 text-[16px] sm:text-[20px] lg:text-[30px] font-medium font-rasa rounded-[80px] transition-all duration-200 ${item === 'About Us' ? 'bg-white text-charcoal' : 'text-[#2F2F2F80]'
                                    }`}
                            >
                                {item}
                            </button>
...
export default IndependentAboutUs