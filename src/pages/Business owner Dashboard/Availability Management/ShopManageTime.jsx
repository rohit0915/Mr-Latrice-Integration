...
const ShopManageTime = () => {
    ...
    const toggleDay = (staffIndex, day) => {
        const updated = [...data];
        updated[staffIndex].workingDays[day] = !updated[staffIndex].workingDays[day];
        setData1(updated);
    };
    ...
    return (
      ...
      ${openDays[data.day] ? 'bg-[#FF827F]' : 'bg-[#D9D9D9] border border-[#2F2F2F]'}`}
                ...
                <div className='mb-5'>
                    <h6 className='block ont-sansation font-[700] sm:text-[25px] text-[20px] text-charcoal mb-5'>Assign Work Days to Each Staff Member</h6>
                    <div className="overflow-x-auto">
                        <table className="min-w-full rounded-lg overflow-hidden  border-collapse">
                            <thead className="bg-[#123E41] text-white text-center">
                                <tr>
                                    ...
                                    {days.map((day) => (
                                        <th key={day} className="p-2 font-semibold uppercase">
                                            {day.slice(0, 3)}
                                        </th>
                                    ))}
                                    <th className="p-2 font-semibold">ADD BREAKS</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white">
                                {data1.map((staff, staffIndex) => (
                                    <tr key={staffIndex} className="text-center border border-[#A8A8A84D]">
                                        ...
                                        {days.map((day) => (
                                            <td key={day} className="p-2">
                                                <input
                                                    type="checkbox"
                                                    className="peer hidden"
                                                    checked={staff.workingDays[day]}
                                                    onChange={() => toggleDay(staffIndex, day)}
                                                />
                                                <div
                                                    className={`w-[24px] h-[24px] mx-auto rounded-sm flex items-center justify-center transition-colors duration-300 cursor-pointer ${staff.workingDays[day]
                                                        ? "bg-[#FF827F]"
                                                        : "bg-[#D9D9D9] border border-[#2F2F2F]"
                                                        }`}
                                                    onClick={() => toggleDay(staffIndex, day)}
                                                >
                                                    {staff.workingDays[day] && (
                                                        <svg
                                        ...
                                        <td className="p-3  flex items-center justify-center cursor-pointer">
                                            <FaPlus color='#000000' size={20} onClick={() => setModalOpen(true)}/>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
