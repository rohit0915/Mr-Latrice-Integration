...
export default function BusinessOwnerBasket() {
    ...
    const [isModalOpen21, setModalOpen21] = useState(false);
    const [isModalOpen22, setModalOpen22] = useState(false);
    const [isModalOpen23, setModalOpen23] = useState(false);
    const [isModalOpen24, setModalOpen24] = useState(false);
    ...
    const DashboardhiringPendingData = [
        {
            ...
            actions: [
                ...
                { label: "Withdraw", onClick: () => setModalOpen20(true) },
    ...
    const DashboardhiringAcceptedData = [
        {
            ...
            actions: [
                ...
                { label: "Cancel", onClick: () => setModalOpen21(true) },
    ...
    const DashboardhiringExpiredData = [
        {
            ...
            "imageUrl": img7,
            actions: [
                { label: "Repost", btnstatus: true, onClick: () => setModalOpen22(true) },
                { label: "Delete", btnstatus: true, onClick: () => setModalOpen23(true) },
        ...
        },
        {
            ...
            actions: [
                { label: "View Details", btnstatus: true, onClick: () => setModalOpen9(true) },
                { label: "Delete", btnstatus: true, onClick: () => setModalOpen23(true) },
                { label: "Create New Request", btnstatus: true, onClick: () => setModalOpen24(true) }
            ]
        },
        {
            "id": 3,
    ...
    const SelectionStageData = [
        {
            ...
            "imageUrl": img12,
            actions: [
                { label: "Withdraw Request", btnstatus: true, onClick: () => setModalOpen20(true)},
                { label: "View Details", btnstatus: true, onClick: () => setModalOpen9(true) },
            ]
        },
        {
        ...
        },
        {
            ...
            "imageUrl": img13,
            actions: [
                { label: "Delete", btnstatus: false, onClick: () => setModalOpen23(true)  },
                { label: "View Details", btnstatus: true, onClick: () => setModalOpen9(true) },
            ]
        },
        {
    ...
    return (
        <BusinessOwnerDashboardLayout title="Basket"
            gpnumber='6'
            titleAction={
        ...
        >
            ...
            />
            <CancelConfirmModal
                isOpen={isModalOpen21}
                onClose={() => setModalOpen21(false)}
            />
            <RepostConfirmModal
                isOpen={isModalOpen22}
                onClose={() => setModalOpen22(false)}
            />
            <DeleteConfirmModal1
                isOpen={isModalOpen23}
                onClose={() => setModalOpen23(false)}
            />
            <CreateNewConfirmModal
                isOpen={isModalOpen24}
                onClose={() => setModalOpen24(false)}
                    ...
                    {activeTab === 'Dashboard' && activeTab1 === 'Hiring Request (s)' && activeTab2 === 'Pending' &&
                        <div className="flex flex-wrap gap-5 sm:flex-row flex-col mt-4">
                            {DashboardhiringPendingData.map((i, idx) => (
                                <div key={idx} className="max-w-md rounded-[10px] border-4 border-[#123E41] bg-white">
                                    {/* Header */}
                                    {isduplicate ? (
                    ...
                    {activeTab === 'Dashboard' && activeTab1 === 'Hiring Request (s)' && activeTab2 === 'Expired' &&
                        <div className="flex flex-wrap gap-5 sm:flex-row flex-col mt-4">
                            {DashboardhiringExpiredData.map((i, index) => (
                                <div key={index} className={`max-w-md rounded-[10px] border-4 border-[#123E41] bg-white`}>
                                    {/* Header */}
                                    <div className="bg-[#D9D9D9] rounded-tl-[7px] rounded-tr-[7px] px-6 py-2 flex gap-0.5 items-center justify-end cursor-pointer">
                                        <TbCopy color="#2F2F2F" />
                                        <h6 className="font-rasa font-[400] sm:text-[17px] text-[15px] text-charcoal">Duplicate</h6>
